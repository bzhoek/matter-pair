...
describe("bitpacking", () => {
  ...
  it("packs test bits", () => {
    const result = packPayloadBits(0b0n, 0x3333n, 0xccccn, 0n, 0n, 0x669n, 0x3ffffffn);
    expect(result.toString(16)).toBe("7fffffecd200666619998");
    expect(result.toString(2)).toBe("11111111111111111111111111011001101001000000000011001100110011000011001100110011000");
    expect(result.toString(2).length).toBe(83);
    const reversed = reverseBytes(result, 11);
    expect(reversed.toString(16)).toBe("989961660620cdfeffff07");
  });
  it("packs food bits", () => {
    const result = packPayloadBits(0n, 0xF00Dn, 0xCAFEn, 0n, 0b00000000n, 0xF00n, 20202021n);
  ...
  it("encodes", () => {
    ...
    const number = ((reversed & 0xffn) << 16n) | ((reversed >> 16n) & 0xffn) | (reversed & 0xff00n);
  ...
  it("encodes as well", () => {
    ...
    const payload = base38encode(result);
